...
for file in os.listdir(NEW_HTML_DIR):
    # Αγνοούμε τα αρχεία archives, το index και ό,τι δεν είναι .html
    if not file.endswith(".html") or "archives" in file or "index" in file:
        continue
        
    full_path = os.path.join(NEW_HTML_DIR, file)
    title, keywords, href, lang, date_obj = extract_html_metadata(full_path)

    # --- Η ΔΙΟΡΘΩΣΗ: Επιβολή γλώσσας βάσει ονόματος αρχείου ---
    ...
    if href in existing_divs:
        ...
        # Δημιουργούμε το HTML σε μια συνεχή γραμμή χωρίς αλλαγές γραμμής ανάμεσα στα tags
        raw_html = (
            f'<div class="link-container" data-title="{title}" data-date="{data_date}" data-tags="{keywords}">'
            f'<a href="{href}" target="_blank" class="link-button">'
            f'<span class="icon">{smart_icon(title, keywords)}</span>&nbsp;{title}'
            f'</a><span class="pub-date">{display_date}</span></div>'
        )
        new_div = BeautifulSoup(raw_html, "html.parser")
...
for lang, path in ARCHIVES.items():
    ...
    if not args.dry_run:
        with open(path, "w", encoding="utf-8") as f:
            # Μετατρέπουμε το soup σε string χωρίς το prettify()
            # Αυτό αφαιρεί τα περιττά κενά που δημιουργούνται από το formatting
f.write("".join(line.strip() for line in str(soup).split("\n"))) 
